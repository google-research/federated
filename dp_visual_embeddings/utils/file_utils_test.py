...
class UtilsForFilesTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_atomic_read(self, name):
    ...
    file_utils.atomic_write_to_csv(dataframe, csv_file)

    dataframe2 = file_utils.atomic_read_from_csv(csv_file)
    pd.testing.assert_frame_equal(dataframe, dataframe2)
