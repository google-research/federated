...
"""Library providing methods for working with the iNaturalist dataset and tasks.
"""
...
# TODO(b/245822977): add unit tests.

# iNaturalist dataset
# Dataset stats: 9275 users, 1203 classes, 120,300 images.
_NUM_LABELS = 1203
...
def _get_inat_embedding_task_datasets(
  ...
  train_data_by_user, test_dataset = tff.simulation.datasets.inaturalist.load_data(
  )

  @tf.function
  def serializable_test_dataset_fn(_: str) -> tf.data.Dataset:
    _, test_dataset = tff.simulation.datasets.inaturalist.load_data()
    return test_dataset
...
class INatTask(task_utils.EmbeddingTask):
  """Variant of the embedding model.  task for the iNaturalist training."""
  ...
  def __init__(self,
    ...
    super().__init__('iNaturalist', datasets)
...
def get_inat_embedding_task(
  ...
  """
  datasets = _get_inat_embedding_task_datasets(
      train_preprocess_spec,
      eval_preprocess_spec,
