...
# The landmark dataset has data for 1262 clients. However, a smaller version of
# it is used in the unit tests, and that one only has data for 233 clients.
_SMALLER_LANDMARK_DATASET_NUM_USERS = 233
...
def _get_expected_data_and_model_spec(image_height_and_width=224):
  return collections.OrderedDict(
      x=collections.OrderedDict(
          images=tf.TensorSpec(
              shape=(None, image_height_and_width, image_height_and_width, 3),
              dtype=tf.float32,
              name=None)),
...
class DatasetLibTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def setUp(self):
    super().setUp()
    train_client_data, test_data = tff.simulation.datasets.gldv2.load_data(
        gld23k=True)
    self.train_client_data = train_client_data
    self.test_data = test_data

  def test_get_landmark_client_data(self):
    ...
    self.assertLen(train_client_data.client_ids,
                   _SMALLER_LANDMARK_DATASET_NUM_USERS)
    self.assertIn(landmark_task._IMAGE_KEY, train_client_dataset.element_spec)
    self.assertIn(landmark_task._IDENTITY_KEY,
                  train_client_dataset.element_spec)

    test_client_dataset = self.test_data
    self.assertIn(landmark_task._IMAGE_KEY, test_client_dataset.element_spec)
    self.assertIn(landmark_task._IDENTITY_KEY, test_client_dataset.element_spec)
...
class TaskLibTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def setUp(self):
    super().setUp()
    preprocess_spec = _get_preprocess_spec()
    self.default_task = landmark_task.get_landmark_embedding_task(
        preprocess_spec, preprocess_spec, gld23k=True)

  @parameterized.named_parameters(('dyn1', 1), ('dyn3', 3))
  def test_get_landmark_embedding_task_datasets(self, dynamic_clients):
    preprocess_spec = _get_preprocess_spec()
    datasets = landmark_task._get_landmark_embedding_task_datasets(
        preprocess_spec,
        preprocess_spec,
        dynamic_clients=dynamic_clients,
        gld23k=True)
    self.assertLen(datasets.train_data.client_ids,
                   _SMALLER_LANDMARK_DATASET_NUM_USERS)
  ...
  def test_get_landmark_embedding_task(self):
  ...
  def test_get_landmark_embedding_task_image_scale(self):
    task = self.default_task
...
class TFFModelTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def _test_model_forward_pass(self, tff_model, num_landmark_labels,
                               dynamic_clients):
    del dynamic_clients
    batch_size = 4
    # Input data is all zero, so the corresponding output is also zero.
    batch_input = _build_fake_inputs(
        batch_size=batch_size, num_landmark_labels=num_landmark_labels)

    batch_output = tff_model.forward_pass(batch_input)
    self.assertAllClose(
        tf.zeros([
            batch_size,
            landmark_task._LANDMARK_NUM_LABELS
        ]), batch_output.predictions[0])
    self.assertAllClose(
        tf.zeros([batch_size, landmark_task._EMBEDDING_DIM_SIZE]),
  ...
  def test_forward_pass_of_tff_model(self, num_landmark_labels,
                                     dynamic_clients):
    task = _get_tff_task_for_testing(dynamic_clients=dynamic_clients)
    self._test_model_forward_pass(
        task.federated_model_fn(),
        num_landmark_labels=num_landmark_labels,
        dynamic_clients=dynamic_clients)

  @parameterized.named_parameters(('statc', 3, 1), ('dync', 1, 3))
  def test_forward_pass_of_embedding_model(self, num_landmark_labels,
                                           dynamic_clients):
    task = _get_tff_task_for_testing(dynamic_clients=dynamic_clients)
    self._test_model_forward_pass(
  ...
  def _test_report_local_unfinalized_metrics(self, tff_model,
                                             num_landmark_labels):
  ...
  def test_report_local_unfinalized_metrics_of_tff_model(self):
    ...
    self._test_report_local_unfinalized_metrics(
        task.federated_model_fn(), num_landmark_labels=3)

  def test_report_local_unfinalized_metrics_of_embedding_model(self):
    task = _get_tff_task_for_testing()
    self._test_report_local_unfinalized_metrics(
        task.embedding_model_fn(), num_landmark_labels=3)
