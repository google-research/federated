...
def _get_optimizer_preconditioner(optimizer: tf.keras.optimizers.Optimizer,
  ...
  if config_name == 'Adagrad':
    ...
    return tf.nest.map_structure(
           lambda a: tf.math.divide_no_nan(1.0, tf.math.sqrt(a) + eps), v)
...
def create_client_update_fn():
  ...
  def client_update(model,
    ...
    avg_local_states = tf.nest.map_structure(tf.zeros_like, model_weights.trainable)
    cum_local_states = tf.nest.map_structure(tf.zeros_like, model_weights.trainable)
    for batch in iter(dataset):
      ...
      cum_local_states = tf.nest.map_structure(
          lambda m, n: m + n, avg_local_states, cum_local_states)
