...
def build_to_ids_fn(
  ...
  def to_ids(example):
    ...
    # Add EOS only when length is shorter than max_sequence_length.
    # Then, pad to the maximum sequence length.
    padding_spec = tf.concat(
        [[[0]], tf.reshape(max_sequence_length - tf.size(tokens) - 1, [1, 1])],
        1)
    tokens = tf.cond(
        tf.less(tf.size(tokens), max_sequence_length),
        lambda: tf.pad(tf.concat([tokens, [eos]], 0), padding_spec),
        lambda: tokens)

    if 'group' in example:
      group = example['group']
    else:
      group = tf.cast(tf.math.equal(example['type'], 'answer'), tf.int64)

    input_x = tf.concat([[bos], tokens], 0)
    input_y = input_x[1:]
    input_x = input_x[:-1]

    return ((input_x, group), input_y)

  return to_ids


def batch_and_split(
    dataset: tf.data.Dataset, sequence_length: int,
...
def create_preprocess_fn(
    ...
    filter_fn=None) -> Callable[[tf.data.Dataset], tf.data.Dataset]:
  ...
  Args:
    ...
    filter_fn: Optional filter function to create a different subpopulation.
