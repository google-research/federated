...
def _reshape_for_character_recognition(element):
  group = tf.cast(element['label'] <= 9, element['pixels'].dtype)
  return ((tf.expand_dims(element['pixels'], axis=-1), group), element['label'])
...
def create_preprocess_fn(
  ...
  Args:
    ...
    filter_fn: The filter function to create day/night domains in the periodic
