...
class PreprocessFnTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_raises_non_iterable_crop(self):
    ...
    with self.assertRaisesRegex(TypeError, 'crop_shape must be an iterable'):
      cifar_classification_preprocessing.create_preprocess_fn(
  ...
  def test_ds_length_is_ceil_num_epochs_over_batch_size(self, num_epochs,
    ...
    preprocess_fn = cifar_classification_preprocessing.create_preprocess_fn(
        preprocess_spec)
  ...
  def test_preprocess_fn_returns_correct_element(self, crop_shape,
    ...
    expected_element = ((tf.zeros(
        ...
        dtype=tf.float32), tf.zeros(shape=(1,), dtype=tf.float32)),
  ...
  def test_preprocess_is_no_op_for_normalized_image(self):
    ...
    simple_example = collections.OrderedDict(image=x, group=0, label=0)
    image_map = cifar_classification_preprocessing.build_image_map(
        crop_shape, distort=False)
    cropped_example = image_map(simple_example)

    self.assertEqual(cropped_example[0][0].shape, crop_shape)
    self.assertAllClose(x, cropped_example[0][0], rtol=1e-03)
