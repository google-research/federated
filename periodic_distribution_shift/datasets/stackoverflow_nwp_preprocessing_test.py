...
class ToIDsFnTest(tf.test.TestCase):
  ...
  def test_ids_fn_truncates_on_input_longer_than_sequence_length(self):
    ...
    to_ids_fn = word_prediction_preprocessing.build_to_ids_fn(
        vocab, max_seq_len)
    data = {'tokens': 'A B C', 'type': 'question'}
    processed = to_ids_fn(data)[0][0]
    self.assertAllEqual(self.evaluate(processed), [bos])
  ...
  def test_pad_token_correct(self):
    ...
    batched_ds = tf.data.Dataset.from_tensor_slices([processed[0][0]
                                                    ]).padded_batch(
                                                        1, padded_shapes=[6])
  ...
  def test_out_of_vocab_tokens_are_correct(self):
    ...
    data = {'tokens': 'A B D', 'type': 'question'}
    processed = to_ids_fn(data)
    self.assertLen(out_of_vocab_tokens, num_out_of_vocab_buckets)
    self.assertIn(self.evaluate(processed)[0][0][3], out_of_vocab_tokens)
...
class PreprocessFnTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_nonpositive_sequence_length_raises(self, sequence_length):
    ...
    with self.assertRaisesRegex(ValueError,
      ...
      word_prediction_preprocessing.create_preprocess_fn(
          preprocess_spec, vocab=['A'], sequence_length=sequence_length)
  ...
  def test_preprocess_fn_returns_correct_sequence_with_3_out_of_vocab_buckets(
    ...
    self.assertEqual(self.evaluate(element[0][0])[0][0], 6)
    self.assertEqual(self.evaluate(element[0][0])[0][1], 1)
    self.assertEqual(self.evaluate(element[0][0])[0][2], 2)
    # OOV is [len(vocab)+1, len(vocab)+2, len(vocab)+3]
    self.assertIn(self.evaluate(element[0][0])[0][3], [3, 4, 5])
    # EOS is len(vocab)+3+2
    self.assertEqual(self.evaluate(element[0][0])[0][4], 7)
    # pad is 0
    self.assertEqual(self.evaluate(element[0][0])[0][5], 0)
