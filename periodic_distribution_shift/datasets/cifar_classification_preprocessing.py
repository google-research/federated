...
"""Preprocessing library for CIFAR classification tasks."""
import collections
...
def build_image_map(
  ...
  def image_map(example):
    ...
    return ((image, example['group']), example['label'])

  return image_map


def cifar10_mapping(x):
  # The labels of CIFAR-100 will be 0 to 99, while labels of CIFAR-10 will be
  # 100 to 109. Orignally labels of CIFAR-10 was 0 to 9, so we add 100.
  return collections.OrderedDict(
      sorted([('image', x['image']),
              ('group', np.ones([], np.float32)),
              ('label', x['label'] + 100)]))


def cifar100_mapping(x):
  return collections.OrderedDict(
      sorted([('image', x['image']),
              ('group', np.zeros([], np.float32)),
              ('label', x['label'])]))
...
def create_preprocess_fn(
    ...
    is_cifar10: bool = False) -> Callable[[tf.data.Dataset], tf.data.Dataset]:
  ...
  Args:
    ...
    is_cifar10: Whether we are preprocessing the CIFAR-10 dataset.
  ...
  def preprocess_fn(dataset):
    ...
    if is_cifar10:
      dataset = dataset.map(cifar10_mapping)
    else:
      dataset = dataset.map(cifar100_mapping)
