...
from periodic_distribution_shift.tasks import cifar_classification_tasks
...
from periodic_distribution_shift.tasks import stackoverflow_nwp_tasks as word_prediction_tasks


TASK_CONSTRUCTORS = {
    'emnist_character':
        emnist_classification_tasks.create_character_recognition_task,
    'stackoverflow_word':
        word_prediction_tasks.create_word_prediction_task,
    'cifar100_10':
        cifar_classification_tasks.create_image_classification_task
}
...
def create_task_from_flags(
...
) -> Union[tff.simulation.baselines.BaselineTask,
  ...
  if hasattr(flags.FLAGS, 'aggregated_kmeans'):
    kwargs['aggregated_kmeans'] = flags.FLAGS.aggregated_kmeans
  if hasattr(flags.FLAGS, 'label_smooth_w'):
    kwargs['label_smooth_w'] = flags.FLAGS.label_smooth_w
  if hasattr(flags.FLAGS, 'label_smooth_eps'):
    kwargs['label_smooth_eps'] = flags.FLAGS.label_smooth_eps
