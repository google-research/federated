...
class DistShiftDatasets(object):
  ...
  Attributes:
    ...
    validation_data_dict: A dict of `tff.simulation.datasets.ClientData` for
      computing validation metrics on different validation subsets. This is the
  ...
  def __init__(self,
    ...
    Args:
      ...
      test_data: An optional `tff.simulation.datasets.ClientData` for computing
        test metrics.
      validation_data_dict: A dict of `tff.simulation.datasets.ClientData` for
        computing validation metrics on different validation subsets.
    ...
    self._validation_data_dict = validation_data_dict
  ...
  @property
  def train_data(self) -> tff.simulation.datasets.ClientData:
  ...
  @property
  def validation_data_dict(self):
    return self._validation_data_dict
  ...
  def _record_dataset_information(self):
    ...
    if self._validation_data_dict is not None:
      for key, val_data in self._validation_data_dict.items():
        if isinstance(val_data, tff.simulation.datasets.ClientData):
          validation_type = 'Federated'
          num_validation_clients = len(val_data.client_ids)
        else:
          validation_type = 'Centralized'
          num_validation_clients = 'N/A'
        dkey = 'validation/{}'.format(key)
        data_info[dkey] = [dkey, validation_type, num_validation_clients]
  ...
  def get_centralized_test_data(self) -> tf.data.Dataset:
    ...
    if isinstance(test_data, tff.simulation.datasets.ClientData):
