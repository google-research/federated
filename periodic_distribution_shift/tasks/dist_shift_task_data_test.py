...
class GetElementSpecTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_get_element_spec(self, dataset_type, preprocess):
    ...
    actual_type = dist_shift_task_data._get_element_spec(data, preprocess_fn)
    self.assertEqual(actual_type, expected_type)


class TaskDatasetsTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_constructs_without_eval_preprocess_fn(self):
    ...
    test_task_data = dist_shift_task_data.DistShiftDatasets(
        train_data=create_client_data(10),
  ...
  def test_record_validation_dataset_info(self, validation_dataset_type,
    ...
    if validation_dataset_type == 'Federated':
      val_data_dict = {
          'set1': create_client_data(num_clients),
          'set2': create_client_data(num_clients)
      }
    elif validation_dataset_type == 'Centralized':
      val_data_dict = {
          'set1': tf.data.Dataset.range(2),
          'set2': tf.data.Dataset.range(2, 4)
      }
    else:
      val_data_dict = None
    test_task_data = dist_shift_task_data.DistShiftDatasets(
        ...
        validation_data_dict=val_data_dict)
    if validation_dataset_type is None:
      ...
      expected_validation_info = [
          'validation/set1', validation_dataset_type, num_clients
      ]
  ...
  def test_data_summary_header_is_constant(self, test_type, validation_type):
    ...
    if validation_type == 'Federated':
      val_data_dict = {
          'set1': create_client_data(4),
          'set2': create_client_data(4)
      }
    elif validation_type == 'Centralized':
      val_data_dict = {
          'set1': tf.data.Dataset.range(7),
          'set2': tf.data.Dataset.range(7)
      }
    else:
      val_data_dict = None
  ...
  def test_summary_gives_correct_validation_information(self, validation_type,
                                                        num_clients):
    if validation_type == 'Federated':
      val_data_dict = {'set1': create_client_data(num_clients)}
    elif validation_type == 'Centralized':
      val_data_dict = {'set1': tf.data.Dataset.range(2)}
    else:
      val_data_dict = None
    test_task_data = dist_shift_task_data.DistShiftDatasets(
        train_data=create_client_data(1),
    ...
    expected_validation_summary = [
        'validation/set1', '|{}'.format(validation_type),
        '|{}'.format(num_clients), '|'
    ]
  ...
  def test_summary_table_structure_with_validation(self):
    ...
    test_task_data = dist_shift_task_data.DistShiftDatasets(
        ...
        validation_data_dict={'set1': train_data})
    data_summary = []
    test_task_data.summary(print_fn=data_summary.append)
