...
The ResNet models can have two branches for the last dense layer, which is
controled by a `dual_branch` flag.
...
def _norm_relu(input_tensor, norm):
  ...
  if norm is NormLayer.group_norm:
    x = tff.simulation.models.group_norm.GroupNormalization(axis=channel_axis)(
        input_tensor)
...
def _shortcut(input_tensor, residual, norm):
  ...
  if stride_width > 1 or stride_height > 1 or not equal_channels:
    ...
    if norm is NormLayer.group_norm:
      shortcut = tff.simulation.models.group_norm.GroupNormalization(
          axis=channel_axis)(
              shortcut)
    elif norm is NormLayer.batch_norm:
...
def create_resnet(input_shape: Tuple[int, int, int],
                  ...
                  dual_branch: bool = False) -> tf.keras.Model:
  ...
  Args:
    ...
    norm_layer: A `NormLayer` describing which normalization layer is used in
      the resulting model.
    dual_branch: Whether to use two branches.
  ...
  if dual_branch:
    ...
    # Output: predictions from both branches, and the features for k-means.
    model = tf.keras.models.Model([img_input, group], preds + [features])
  else:
    ...
    model = tf.keras.models.Model([img_input, group], x)
  return model


def create_resnet18(input_shape: Tuple[int, int, int],
                    num_classes: int,
                    norm_layer: NormLayer = NormLayer.group_norm,
                    dual_branch: bool = False) -> tf.keras.Model:
...
def create_resnet34(input_shape: Tuple[int, int, int],
                    num_classes: int,
                    norm_layer: NormLayer = NormLayer.group_norm,
                    dual_branch: bool = False) -> tf.keras.Model:
...
def create_resnet152(input_shape: Tuple[int, int, int],
                     num_classes: int,
                     norm_layer: NormLayer = NormLayer.group_norm,
                     dual_branch: bool = False) -> tf.keras.Model:
