...
_PREFIX_RVAL = (
    _prefix_sum_matrix(_DIMENSION, tf.float32),
    tf.eye(_DIMENSION),
    None,
)

_LR_MOMENTUM_0p90_RVAL = (
    _momentum_matrix(_DIMENSION, tf.float32, 0.9),
    tf.eye(_DIMENSION),
    tf.ones(_DIMENSION),
)

_LR_MOMENTUM_0p00_RVAL = (
    _momentum_matrix(_DIMENSION, tf.float64, 0.0),
    tf.eye(_DIMENSION, dtype=tf.float64),
    None,
)
...
class AggregatorBuilderTest(tf.test.TestCase):
  ...
  def test_tree_aggregator_constructs(self):
    ...
    self.assertIsInstance(
        aggregator, tff.aggregators.UnweightedAggregationFactory
    )

  def test_dp_sgd_constructs(self):
    aggregator = _build_aggregator_by_method(aggregator_method='dp_sgd')
    self.assertIsInstance(
        aggregator, tff.aggregators.UnweightedAggregationFactory
    )

  @mock.patch.object(
      matrix_io, 'load_w_h_and_maybe_lr', return_value=_PREFIX_RVAL
  )
  @flagsaver.flagsaver(matrix_root_path='/root')
  ...
  @mock.patch.object(
      matrix_io, 'load_w_h_and_maybe_lr', return_value=_LR_MOMENTUM_0p00_RVAL
  )
  @flagsaver.flagsaver(matrix_root_path='/root')
  ...
  @mock.patch.object(
      matrix_io, 'load_w_h_and_maybe_lr', return_value=_LR_MOMENTUM_0p90_RVAL
  )
  @flagsaver.flagsaver(matrix_root_path='/root')
  ...
  def test_raises_momentum_mismatch(self):
    with self.assertRaisesRegex(
        ValueError, r'Mismatch between inferred momentum'
    ):
  ...
  @flagsaver.flagsaver(matrix_root_path='/root')
  def test_raises_bad_sensitivityh(self, mock_fn):
    def bad_sensitivity(h_matrix):
      raise ValueError('sensitivity is too high!')

    with self.assertRaisesRegex(ValueError, r'sensitivity is too high!'):
      _build_aggregator_by_method(
          aggregator_method='opt_momentum_matrix',
          momentum=0.0,
          verify_sensitivity_fn=bad_sensitivity,
      )
