...
class DirichletCoarseTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_synthesize_by_dirichlet_over_coarse_and_fine_labels(
      self, num_clients, rotate):
    test_dataset = tf.data.Dataset.from_tensor_slices(
        collections.OrderedDict(
            x=list(range(9)),
            foo=['a', 'b', 'c'] * 3,
            label=[0, 1, 6] * 3,
            coarse_label=[0, 0, 1] * 3))
    cd = coarse_dirichlet.synthesize_by_dirichlet_over_coarse_and_fine_labels(
        test_dataset, num_clients, use_rotate_draw=rotate)
