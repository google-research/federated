...
_test_measurements_type_struct_float_tensor = (tf.int32, (_nbins,))
_test_measurements_type_struct_list_float_tensors = [(tf.int32, (_nbins,)),
                                                     (tf.int32, (_nbins,))]
...
class HistogramWeightsComputationTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_histogram_type_properties(self, value_type,
                                     expected_measurements_type):
    factory = _histogram_weights()
...
class HistogramWeightsExecutionTest(tf.test.TestCase, parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ('struct_list_float_scalars', _test_value_type_struct_float_tensor,
       [[1.0, 2.0, 3.0], [1.0, 2.0, 3.0]], [4, 2, 0, 0], [2.0, 4.0, 6.0]),
      ('struct_list_float_tensors', _test_value_type_struct_list_float_tensors,
       [[[1.0, 1.0], [1.0, 2.0, 3.0]], [[9.0, 9.0], [1.0, 2.0, 3.0]]],
       [[2, 0, 0, 2], [4, 2, 0, 0]], [[10.0, 10.0], [2.0, 4.0, 6.0]]))
  def test_histogram_impl(self, value_type, client_values, expected_histogram,
                          expected_result):
    value_type = tff.to_type(value_type)
    histogram_agg = _histogram_weights().create(value_type)
    state = histogram_agg.initialize()
    histogram = histogram_agg.next(state, client_values).measurements
    self.assertAllEqual(histogram, expected_histogram)
    result = histogram_agg.next(state, client_values).result
