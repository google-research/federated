...
def build_hypcluster_train(
  ...
  *   `get_model_weights`: A `tff.Computation` with type signature `(S -> M)`,
  ...
  def model_variables_exclude_last_layer(
      model: tff.learning.Model) -> List[List[tf.Variable]]:
    # Only works for models built via `tff.learning.from_keras_model`.
    return [layer.weights for layer in model._keras_model.layers[:-1]]  # pylint:disable=protected-access

  @tf.function
  def create_initial_weights_only_differ_last_layer(
      model_list: List[tff.learning.Model]) -> List[tff.learning.ModelWeights]:
    """Returns a list of model weights with shared N-1 layers."""
    first_model = model_list[0]
    # Extract the values of the first model's N-1 layers.
    shared_values_exclude_last_layer = tf.nest.map_structure(
        lambda var: var.read_value(),
        model_variables_exclude_last_layer(first_model))
    # Assign the values of the first model's N-1 layers to the rest models.
    for model in model_list[1:]:
      tf.nest.map_structure(lambda a, b: a.assign(b),
                            model_variables_exclude_last_layer(model),
                            shared_values_exclude_last_layer)
    return [tff.learning.ModelWeights.from_model(model) for model in model_list]
