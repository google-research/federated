...
def encoder(seed, x, k, epsilon, number_candidates):
  ...
  c1 = np.exp(epsilon) / (
      sc.comb(k - 1, d - 1) * np.exp(epsilon) + sc.comb(k - 1, d))
...
def get_approx_epsilon(epsilon_target, k, number_candidates, delta):
  ...
  for _, epsilon in enumerate(epsilon_search_space):
    ...
    t = np.abs(cupper - clower) * np.sqrt(
        (np.log(2 / delta)) / (2 * number_candidates))
    if -1 < t < 1:
...
def encode_decode_miracle_fast(seed, x, k, epsilon, number_candidates):
  """A fast implementation of the miracle protocol.

  Instead of generating number_candidates samples, generate one sample with true
  expectation.
  ...
  pi_in = np.exp(epsilon) / (
      num_cand_in_cap * np.exp(epsilon) + (number_candidates - num_cand_in_cap))
...
def encode_decode_modified_miracle_fast(seed, x, k, epsilon, number_candidates):
  """A fast implementation of the modified miracle protocol.
  ...
  pi_in = c1 / (
      num_cand_in_cap * c1 + (number_candidates - num_cand_in_cap) * c2)
  tilde_pi_in = pi_in * (1 + beta *
                         (np.exp(epsilon) - 1)) / ((1 + expected_beta *
                                                    (np.exp(epsilon) - 1)))
  if beta > expected_beta:
    tilde_pi_in = tilde_pi_in * (beta + expected_beta *
                                 (np.exp(epsilon) - 1)) / (
                                     beta * np.exp(epsilon))
