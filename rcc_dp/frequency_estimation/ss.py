...
def encode_string_fast(k, epsilon, x):
  ...
  # Instead of selecting exactly d ones, set each bit to be one independently
...
def decode_string(k, epsilon, z, length, normalization=1):
  ...
  temp1 = ((k - 1) * np.exp(epsilon) + 1.0 * (k - 1) *
           (k - d) / d) / ((k - d) * (np.exp(epsilon) - 1))
  temp2 = ((d - 1) * np.exp(epsilon) + k - d) / (1.0 * (k - d) *
                                                 (np.exp(epsilon) - 1))
