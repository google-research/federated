...
This includes things like the evaluation of miracle, rhr, subset selection
methods when either the data dimension k, the number of users n, or the privacy
    ...
    for itr in range(config.num_itr):
      ...
      if config.run_miracle:
        ...
        for i in range(n):
          if config.encoding_type == "fast":
            x_miracle[:, i] = miracle.encode_decode_miracle_fast(
                i + itr * n, x[i], k, epsilon_target / 2, 2**coding_cost)
          else:
            _, _, index = miracle.encoder(i + itr * n, x[i], k,
                                          epsilon_target / 2, 2**coding_cost)
            x_miracle[:,
                      i] = miracle.decoder(i + itr * n, index, k,
                                           epsilon_target / 2, 2**coding_cost)
        prob_miracle = unbias.unbias_miracle(
            k,
            epsilon_target / 2,
            2**coding_cost,
            x_miracle.T,
            n,
            normalization=1)
        miracle_error[itr, step] = np.linalg.norm(
            [p_i - phat_i for p_i, phat_i in zip(prob, prob_miracle)], ord=1)
      ...
      if config.run_approx_miracle:
        ...
        epsilon_approx = miracle.get_approx_epsilon(epsilon_target, k,
                                                    2**approx_coding_cost,
                                                    delta)
        for i in range(n):
          if config.encoding_type == "fast":
            x_approx_miracle[:, i] = miracle.encode_decode_miracle_fast(
                i + itr * n, x[i], k, epsilon_approx, 2**coding_cost)
          else:
            _, _, index = miracle.encoder(i + itr * n, x[i], k, epsilon_approx,
                                          2**coding_cost)
            x_approx_miracle[:, i] = miracle.decoder(i + itr * n, index, k,
                                                     epsilon_approx,
                                                     2**coding_cost)
        prob_approx_miracle = unbias.unbias_miracle(
            k,
            epsilon_approx,
            2**coding_cost,
            x_approx_miracle.T,
            n,
            normalization=1)
        approx_miracle_error[itr, step] = np.linalg.norm(
            [p_i - phat_i for p_i, phat_i in zip(prob, prob_approx_miracle)],
            ord=1)

      if config.run_ss:
        x_ss = ss.encode_string_fast(k, epsilon_target, x)
        prob_ss = ss.decode_string(k, epsilon_target, x_ss, n, normalization=1)
        ss_error[itr, step] = np.linalg.norm(
      ...
      if config.run_rhr:
        ...
        prob_rhr = rhr.decode_string_fast(
            k, epsilon_target, coding_cost, x_rhr,
            normalization=1)  # estimate the original underlying distribution
        rhr_error[itr, step] = np.linalg.norm(
            [p_i - phat_i for p_i, phat_i in zip(prob, prob_rhr)], ord=1)

    if config.run_approx_miracle:
      print("approx miracle error: " + str(approx_miracle_error[:, step]))
    if config.run_miracle:
      print("miracle error: " + str(miracle_error[:, step]))
    if config.run_modified_miracle:
      print("modified miracle error: " + str(modified_miracle_error[:, step]))
    if config.run_ss:
