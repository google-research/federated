...
def encode_string(dim, epsilon, comm, x):
  ...
  z = rr_encode_string(int(math.pow(2, eff_comm)), epsilon, np.array(loc_sign))
  return z
...
def rr_encode_string(alphabet_size, epsilon, samples):
  ...
  flip = np.random.random_sample(n) < (alphabet_size - 1) / (
      math.exp(epsilon) + alphabet_size - 1)
...
def decode_string_fast(dim, epsilon, comm, z, normalization=1):
  ...
  for g_idx in range(block_size):
    ...
    histograms[g_idx] = histograms[g_idx] * (math.exp(epsilon) + math.pow(
        2, eff_comm) - 1) / (math.exp(epsilon) - 1) * (
            block_size / n)

  # Obtain estimator of q.
  q = np.zeros((block_size, int(padded_d / block_size)))
  for j in range(block_size):
    q[j, :] = fast_inverse_hadamard_transform(
        int(padded_d / block_size), histograms[j, :])
