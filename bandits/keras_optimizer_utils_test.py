...
def setUpModule():
  ...
  keras_optimizer_utils.define_optimizer_flags(TEST_SERVER_FLAG_PREFIX)
  keras_optimizer_utils.define_optimizer_flags(TEST_CLIENT_FLAG_PREFIX)
...
_OPTIMIZERS_TO_TEST = [
    (name, name, cls)
    for name, cls in keras_optimizer_utils._SUPPORTED_OPTIMIZERS.items()
]
...
class UtilsTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_create_optimizer_fn_from_flags_invalid_optimizer(self):
    ...
    with self.assertRaisesRegex(ValueError, 'not a valid optimizer'):
      keras_optimizer_utils.create_optimizer_fn_from_flags(
  ...
  def test_create_client_optimizer_from_flags(
      self, optimizer_name, optimizer_cls
  ):
    commandline_set_learning_rate = 100.0
    with flag_sandbox({
        '{}_optimizer'.format(TEST_CLIENT_FLAG_PREFIX): optimizer_name,
        '{}_learning_rate'.format(
            TEST_CLIENT_FLAG_PREFIX
...
class RemoveUnusedFlagsTest(tf.test.TestCase):
  ...
  def test_remove_unused_flags_without_optimizer_flag(self):
    hparam_dict = collections.OrderedDict(
        [('client_opt_fn', 'sgd'), ('client_sgd_momentum', 0.3)]
    )
    with self.assertRaisesRegex(
        ValueError, 'The flag client_optimizer was not defined.'
    ):
      _ = keras_optimizer_utils.remove_unused_flags('client', hparam_dict)
  ...
  def test_remove_unused_flags_with_empty_optimizer(self):
    ...
    with self.assertRaisesRegex(
        ValueError,
        (
            'The flag optimizer was not set. '
            'Unable to determine the relevant optimizer.'
        ),
    ):
      _ = keras_optimizer_utils.remove_unused_flags(
  ...
  def test_remove_unused_flags_with_prefix(self):
    ...
    relevant_hparam_dict = keras_optimizer_utils.remove_unused_flags(
        'client', hparam_dict
    )
    expected_flag_names = [
        'client_optimizer',
