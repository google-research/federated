...
class PreprocessFnTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_label_distribution_shift(self):
    word_vocab = ['A', 'B', 'C']
    tag_size, data_size = 20, 10
    top_tag_size = int(
        tag_size * (1 - tag_prediction_preprocessing.LABEL_DIST_SHIFT_RATIO)
    )
    tag_vocab = [str(i) for i in range(tag_size)]
      ...
      return tf.strings.join(
          [
              tf.strings.as_string(
                  tf.random.uniform(
                      [], minval=0, maxval=top_tag_size, dtype=tf.int32
                  )
              ),
              tf.strings.as_string(
                  tf.random.uniform(
                      [], minval=top_tag_size, maxval=tag_size, dtype=tf.int32
                  )
              ),
          ],
          separator='|',
    ...
    tags = [random_tag() for _ in range(data_size)]
    test_data = collections.OrderedDict(
        creation_date=(['unused date'] * data_size),
        score=([tf.constant(0, dtype=tf.int64)] * data_size),
        tags=(tags),
        title=(['C'] * data_size),
        tokens=(['A'] * data_size),
        type=(['unused type'] * data_size),
    )
    ds = tf.data.Dataset.from_tensor_slices(test_data)
    preprocess_spec = tff.simulation.baselines.ClientSpec(
        num_epochs=1, batch_size=4, shuffle_buffer_size=1
    )
    normal_preprocess_fn = tag_prediction_preprocessing.create_preprocess_fn(
        preprocess_spec,
    ...
    for _, label in preprocessed_ds:
      label_sum = tf.reduce_sum(label, axis=0)
      self.assertEqual(tf.shape(label_sum), [tag_size])
      self.assertNotAllEqual(label_sum[:top_tag_size], tf.zeros([top_tag_size]))
      self.assertNotAllEqual(
          label_sum[top_tag_size:], tf.zeros([tag_size - top_tag_size])
      )
    shift_preprocess_fn = tag_prediction_preprocessing.create_preprocess_fn(
        preprocess_spec,
        word_vocab=word_vocab,
        tag_vocab=tag_vocab,
